...
def data():
    """过滤查询"""
    student_list = Student.query.filter(Student.name.contains("小")).all()
    print(student_list)

    student_list = Student.query.filter(Student.name.startswith("小")).all()
    print(student_list)

    student_list = Student.query.filter(Student.name.endswith("小")).all()
    print(student_list)
    ...
    """精确查询"""
    student_list = Student.query.filter_by(age=5).all()
    print(student_list)
